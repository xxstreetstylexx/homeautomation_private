    ...
    $.ajax({
        ...
        $.ajax({
            ...
            for (i = 0; i < res.automatic.length; i++) {
                var $class;
                var index = i;
                var targetName = res.automatic[i].target;
                for (z = 0; z < res.automatic[i].actions.length; z++) {
                    if (res.automatic[i].actions[z].active) {
                    ...
                    $btn = $('<button class="btn ' + $class + '" data-automaticswitch="' + res.automatic[i].actions[z].id + '" type="button">')
                            .html($status)
                            ;

                    if (res.automatic[i].actions[z].sensor_type == 'ZHATemperature') {
                        $value = res.automatic[i].actions[z].value / 100 + ' °C';
                    } else {
                        $value = res.automatic[i].actions[z].value;
                    }
                    $('<td>').html(res.automatic[i].actions[z].sensor_name).appendTo($tr);
                    $('<td>').html(res.automatic[i].actions[z].operation).appendTo($tr);
                    $('<td>').html($value).appendTo($tr);
                    $('<td>').html(res.automatic[i].target).appendTo($tr);
                    $('<td>').html(res.automatic[i].actions[z].start + ' - ' + res.automatic[i].actions[z].end).appendTo($tr);
                    $('<td>').append($btn).appendTo($tr);

                    Progress.find('#gsetting-automatics').append($tr);
                }
            }
